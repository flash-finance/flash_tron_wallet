...
class TronTransaction {
  ...
  Future<bool> transTrx(BuildContext context, String fromAddress, String toAddress, Int64 amount) async {
    ...
    try {
      ...
      TronMsgSignature msgSignature2 = TronMsgSignature(intToBytes(msgSignature.r), intToBytes(msgSignature.s), msgSignature.v);
  ...
  Future<bool> transTrc10(BuildContext context, String assetID, String fromAddress, String toAddress, Int64 amount) async {
    print('TronTransaction transTrc10');
    String tronGrpcIP = Provider.of<HomeProvider>(context, listen: false).tronGrpcIP;
    final channel = ClientChannel(
      tronGrpcIP.trim(),
      port: 50051,
    ...
    Uint8List originToAddress = base58.decode(toAddress).sublist(0, 21);
    try {
      final tac = TransferAssetContract();
      tac.assetName = utf8.encode(assetID);
      tac.ownerAddress = originFromAddress;
      tac.toAddress = originToAddress;
      tac.amount = amount;

      final trxResult = await stub.transferAsset2(tac);
      Transaction transaction = trxResult.transaction;
      Transaction_raw rawData = trxResult.transaction.rawData;
      Uint8List hash = sha256.convert(rawData.writeToBuffer()).bytes;
      ...
      if (result.toProto3Json().toString().contains('true') == true) {
        ...
        print('transTrc10 result error msg: ${utf8.decode(result.message)}');
        await channel.shutdown();
        return false;
      }
    } catch (e) {
  ...
  Future<bool> transTrc20(BuildContext context, String contractAddress, String fromAddress, String toAddress, int amount) async {
    ...
    final stub = WalletClient(channel);
    try {
      SmartContract response = await stub.getContract(BytesMessage()..value = base58.decode(contractAddress).sublist(0, 21));
  ...
  Future<bool> trc20Approve(BuildContext context, String contractAddress, String fromAddress, String toAddress, int amount) async {
  ...
  Future<bool> trc20DexOrder(BuildContext context, String contractAddress, int orderType,
      String userAddress, String quoteTokenAddress, int quoteAmount, String baseTokenAddress, int baseAmount,
      int tradePrice, int channelID) async {
  ...
  Future<bool> trc20DexCancel(BuildContext context, String contractAddress, String userAddress, int orderID) async {
  ...
  Future<bool> execute(WalletClient stub, String hexPrivateKey, String base58OwnerAddress, String base58ContractAddress, Uint8List dataList, int callValue) async {
    ...
    Transaction trans = await buildTransaction(stub, req, hexPrivateKey, Transaction_Contract_ContractType.TriggerSmartContract, dataList);
  ...
  Future<Transaction>  buildTransaction(WalletClient stub, TriggerSmartContract req, String hexPrivateKey, Transaction_Contract_ContractType ctxType, Uint8List data) async {
    ...
    trans.rawData.refBlockBytes = resp.blockHeader.rawData.number.toBytes().reversed.toList().sublist(6, 8);
    trans.rawData.refBlockHash = sha256.convert(resp.blockHeader.rawData.writeToBuffer()).bytes.sublist(8, 16);
    trans.rawData.timestamp = Int64(DateTime.now().toUtc().millisecondsSinceEpoch);
    trans.rawData.feeLimit = Int64(10 * 1000000);
    trans.rawData.expiration = Int64(DateTime.now().toUtc().add(Duration(seconds: 180)).millisecondsSinceEpoch);
